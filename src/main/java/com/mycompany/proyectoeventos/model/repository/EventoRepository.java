...
public interface EventoRepository extends JpaRepository<Evento, Integer> {
    ...
    @Query(value = "select * FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.fecha >= curdate() and e.idCreador in (select v.idCreador from evento v where v.idCreador = ?2) order by e.fecha ASC, e.idEvento ASC limit ?3,?4", nativeQuery = true)
    public List<Evento> findBuscadorMisEventosAfter(String nombre, int id, int inicio, int tamaño);

    @Query(value = "select count(*) FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.fecha >= curdate() and e.idCreador in (select v.idCreador from evento v where v.idCreador = ?2)", nativeQuery = true)
    public int contadorMisEventosAfter(String nombre, int id);

    @Query(value = "select * FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.fecha <= curdate() and e.idCreador in (select v.idCreador from evento v where v.idCreador = ?2) order by e.fecha ASC, e.idEvento ASC limit ?3,?4", nativeQuery = true)
    public List<Evento> findBuscadorMisEventosBefore(String nombre, int id, int inicio, int tamaño);

    @Query(value = "select count(*) FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.fecha <= curdate() and e.idCreador in (select v.idCreador from evento v where v.idCreador = ?2)", nativeQuery = true)
    public int contadorMisEventosBefore(String nombre, int id);
    ...
    @Query(value = "select * FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.idCategoria in ( select v.idCategoria from evento v where v.fecha >= curdate() and v.idCategoria = ?2) order by e.fecha ASC, e.idEvento ASC limit ?3,?4 ", nativeQuery = true)
    public List<Evento> findBuscadorPorCategoria(String nombre, int categoria, int inicio, int tamaño);

    @Query(value = "select count(*) FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.idCategoria in ( select v.idCategoria from evento v where v.fecha >= curdate() and v.idCategoria = ?2)", nativeQuery = true)
    public int contadorBuscadorPorCategoria(String nombre, int categoria);

    // Funciones Para EVENTOS ASISTIDOS
    @Query(value = "select count(*) from evento e inner join asistenciaevento x on (e.idEvento = x.Evento_idEvento) where e.fecha >= curdate() and x.Usuario_idUsuario = ?1", nativeQuery = true)
    ...
    @Query(value = "select * FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.idEvento in (select e.idEvento from evento e inner join asistenciaevento x on (e.idEvento = x.Evento_idEvento) where e.fecha >= curdate() and x.Usuario_idUsuario = ?2) order by e.fecha ASC, e.idEvento ASC limit ?3,?4", nativeQuery = true)
    public List<Evento> findBuscadorAfter_Asistencias(String nombre, int id, int inicio, int tamaño);

    @Query(value = "select count(*) FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.idEvento in (select e.idEvento from evento e inner join asistenciaevento x on (e.idEvento = x.Evento_idEvento) where e.fecha >= curdate() and x.Usuario_idUsuario = ?2)", nativeQuery = true)
    public int ContadorBuscadorAfter_Asistencias(String nombre, int id);

    @Query(value = "select * FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.idEvento in (select e.idEvento from evento e inner join asistenciaevento x on (e.idEvento = x.Evento_idEvento) where e.fecha <= curdate() and x.Usuario_idUsuario = ?2) order by e.fecha ASC, e.idEvento ASC limit ?3,?4", nativeQuery = true)
    public List<Evento> findBuscadorBefore_Asistencias(String nombre, int id, int inicio, int tamaño);

    @Query(value = "select count(*) FROM evento e WHERE e.nombre LIKE CONCAT('%',?1,'%') and e.idEvento in (select e.idEvento from evento e inner join asistenciaevento x on (e.idEvento = x.Evento_idEvento) where e.fecha <= curdate() and x.Usuario_idUsuario = ?2)", nativeQuery = true)
    public int ContadorBuscadorBefore_Asistencias(String nombre, int id);
}
